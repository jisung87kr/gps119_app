...
## 0. 지금 떠 있는 것 (최초 설치 2026-08-10 · **최종 배포 2026-09-07 `308da4d`**)
...
| 스케줄러 | `schedule:run` 매분. 현재 `location:purge`(위치 이력 180일 파기) 매일 **04:40 KST** — 2026-09-07 앱 시간대 전환 전에는 실제로 04:40 UTC(13:40 KST)에 돌았다 |
...
| 2026-08-13 | `5c5c620` | PR #20 — 행사 참가자의 신고가 「상시 운영」으로 새던 것 + 접수 대상 UI. 마이그레이션 없음 |
| 2026-08-13 | `143316b` | PR #21 — **역할을 «사람»이 아니라 «행사»별로 판정.** 마이그레이션 1건(`last_entered_at`) |
| 2026-09-01 | `5530b00` | PR #22~#33 묶음 — 위치 권한 3축(M-5)·브리지·관제 추적 배지·iOS 잠금화면 푸시·행사 복제 500 + Universal/App Links + **`deploy.sh` 에 Apache 반영 단계 신설.** 마이그레이션 1건(`location_permission`) |
| 2026-09-01 | `7f4a13f` | 지령 화면·셸 전송기가 «웹» 경로로 돌던 것(화면을 끄면 위치가 끊김). 마이그레이션 없음 |
| 2026-09-01 | `3a37f9a` | 약관 동의 «수집 시점» 수취(위치정보법)·웹뷰 JS 에러 보고(M-16)·관제 이동 궤적(M-25)·백그라운드 위치 사전 고지·새 아이콘. 마이그레이션 1건(`user_consents`) |
| 2026-09-01 | `3498a47`→`7207196` | 실기기 수정 5건을 20:03~20:31 KST 에 연속 배포 — 지도 시간 초과, 위치 문구, 안드로이드 내비게이션바 가림 3건. 마이그레이션 없음 |
| 2026-09-01 | `2d99397` | 배터리 최적화 예외를 앱이 안내(M-26). 마이그레이션 없음 |
| 2026-09-02 | `61d6530` | 바텀시트를 상태별로 가름(iOS 하단 빈 공간). 마이그레이션 없음 |
| 2026-09-04 | `d2badc5` | 명단 CSV **구분자 자동 감지**(탭·세미콜론) + UTF-16 — 탭 파일 161행이 전부 «전화번호 비어 있음»으로 실패하던 것. 마이그레이션 없음 |
| 2026-09-04 | `eb2cf8b` | 앱 알림 토글이 페이지 이동에 «꺼짐»으로 읽히던 것(켜짐 기록을 localStorage 에) · 페이지별 Vue 루트 v-cloak · 관제 역할 배정 패널 이름·역할 검색. 마이그레이션 없음 |
| 2026-09-04 | `345ac49` | 운영진 회원계정 일괄 발급(ADR-0009) — 초기 비밀번호 password, 첫 로그인 강제 변경+동의. 마이그레이션 1건(`must_change_password`·`issued_at`·`issued_by`) |
| 2026-09-04 | `5f0bc2b` | 관제 지도 우클릭 메뉴(위치정보·좌표 복사, 공유, 카카오맵 열기). 마이그레이션 없음. **이 행은 09-07 에 서버 `git reflog`(18:25 KST checkout)로 복원했다** — 배포 직후 기록을 또 빠뜨렸다 |
| 2026-09-07 | `6c2066a` | PR #34 — 현장 피드백 F-04~F-12 웹분: 역할 공무원·회송팀, 발급 비밀번호 전화번호, 참가자 추가 검색·역할순 정렬, 카카오 길안내 폴백, iOS 위치 팝업 반복, 지령 알림음 사이렌, **푸시 페이로드 채널 v2·번들 사운드(셸 빌드 3 과 한 쌍, 웹 먼저)**. 마이그레이션 없음. 21:02 KST, 접속 0건일 때 |
| 2026-09-07 | `ea993dc` | PR #37 — **푸시 등록을 로그인한 사람에게 맞춘다**(F-13). 현장 테스트에서 구급대 계정에 배정해도 알림이 안 왔다: 같은 기기를 먼저 쓴 계정의 켜짐 기록·구독이 남아 서버 토큰이 그 사람 소유였다. 켜짐 기록 값을 사용자 id 로, 레이아웃 `gps119-user` meta, 페이지마다 주인 동기화. 마이그레이션 없음. 21:43 KST |
| 2026-09-07 | `cb413bf` | PR #39·#41 — FCM 거절 응답 사유 로깅 + 알림 켜기 때 권한을 항상 재요청(소리 없이 허가된 아이폰). 마이그레이션 없음. 22:20 KST. **이 배포 뒤 22:25 에 운영 대상 iOS 앱 푸시 종단(수신·소리)이 처음 확인됐다** — 코드가 아니라 APNs 키 교체(§5)와 기기 재설치가 결정적이었다 |
| 2026-09-07 | `d5c8694` | PR #43 — 알림 권한이 정해지기 전엔 뱃지를 안 건드린다. 뱃지 플러그인 `clear()` 가 iOS 에서 배지만으로 권한을 요청해 새 설치의 첫 프롬프트가 「배지만」이 됐고(설정 목록에 GPS119 만 「배지」), 그 뒤 소리·배너 요청을 iOS 가 무시했다. 마이그레이션 없음. 22:30 KST |
| 2026-09-07 | `9f83a30` | PR #46 — Android 포그라운드 로컬 알림에 상태바 아이콘·색(`ic_stat_gps119`, brand-600)을 넘긴다. 안 넘기면 플러그인 기본 아이콘. 마이그레이션 없음. 22:50 KST |
| 2026-09-07 | `f45b913` | PR #48 — 기기 등록에 앱 버전·빌드(`app_version`, `App.getInfo()`)를 싣는다. 아이콘이 Capacitor 로고로 온 것을 보고서야 「그 폰은 빌드 1」임을 알았다. 마이그레이션 없음. 23:00 KST |
| 2026-09-07 | `308da4d` | PR #55 — 2차 피드백: **앱 시간대 UTC→Asia/Seoul + MySQL 세션 +09:00**(시각 열이 전부 TIMESTAMP 라 데이터 이관 없음; 배포 직후 지령 #51 `assigned_at` 이 22:25 KST 로 읽히는 것 확인), 관제 신고 접수 토스트+알림음, 앱용 알림 거부 문구, 역할 배정 목록 높이. 마이그레이션 없음. 23:40 KST. ⚠️ 이 배포부터 스케줄 `dailyAt('04:40')` 이 진짜 04:40 KST 다 |
| 2026-09-08 | `2e2da7f` | PR #60 — **회원 탈퇴를 익명화로**(ADR-0010, `AccountDeletionService`): 지령 이력이 있는 구급대원·상황실 계정은 삭제가 FK RESTRICT 로 500 이었다. **마이그레이션 1건**(`users.account_deleted_at` 추가, `phone` nullable). `review:demo` 명령 추가 — 배포 직후 운영에서 실행해 심사용 행사 #5 + 데모 계정 3종 생성(비밀번호는 콘솔에만). 접속 0건 시점, 20:17 KST |
| 2026-09-08 | `f042687` | PR #62 — `/control` 백링크: 관리자는 대시보드, 행사 상황실은 **마이페이지**. 상황실 계정이 관제에 갇혀 프로필·탈퇴에 못 가던 것(심사 데모 계정에서 발견). 마이그레이션 없음. 20:31 KST. 반영 증명: 서빙 번들 `main-TWR0JLe5.js` 에 `backLabel` |

> ⚠️ **08-13 두 건은 3주 가까이 기록되지 않은 채로 있었다.** 그동안 이 표와 `CLAUDE.md` 는
> 마지막 배포를 `226f5b7` 로 가리켰고, 그래서 「PR #20·#21 은 아직 미배포」라는 판단이 나왔다.
> **배포는 `deploy.sh` 가 하지만 이 표는 사람이 채운다** — 배포 직후 여기에 한 줄을 남긴다.
> 뒤늦게 복원할 때의 근거는 서버의 `.deploy/last-release`(직전 릴리스 SHA)와
> `backups/` 의 **배포마다 뜨는 덤프 타임스탬프**다. 2026-08-31 에 그 둘로 재구성했다.
> 09-01~09-02 의 10건도 똑같이 빠졌다가 09-04 배포 때 복원했다 — 이번엔 서버의 `git reflog`
> (checkout 기록) 를 썼고, 셋 중 이게 가장 정확하다(SHA 와 시각이 한 줄에 같이 남는다).
...
**2026-08-13 배포(`5c5c620` → `143316b`)에서 확인한 것** — 2026-08-31 서버 실측으로 소급 확인.
- `2026_08_13_100000_add_last_entered_at_to_event_participants_table` **batch 4 적용됨**
- 컨테이너 안 `app/Models/User.php` 에 `canFileRequestFor` 존재 — 코드가 실제로 갈렸다
- `public/build/manifest.json` 이 배포 시각(08-12 16:15Z)과 일치 — Vite 산출물도 갱신됐다
- 컨테이너 4개 running · 점검 모드 아님 · 실패 잡 0 · `/up` 200
- 큐 워커 `RestartCount` 가 수백 대로 보이지만 **정상이다.** `--max-time=3600` 때문에 매시
  정각에 스스로 끝나고 `restart: unless-stopped` 가 다시 띄운다(`ExitCode=0`, OOM 아님).
  크래시 루프와 구별하려면 **재시작 간격이 1시간인지**와 종료코드를 본다.
...
**큐 워커·Reverb 재기동** → **Apache 설정 반영(graceful)** → 점검 해제 → `/up` 헬스체크.

> 🔑 **Apache 반영이 왜 별도 단계인가** — `docker/apache/` 는 컨테이너에 마운트돼 있어
> 체크아웃만으로 «파일»은 바뀌지만, 떠 있는 Apache 는 옛 설정을 메모리에 들고 계속 돈다.
> 🔴 실패가 조용하다: `apple-app-site-association` 의 `ForceType` 이 안 먹으면
> Content-Type 없이 나가고 **iOS 가 그 파일을 무시한다** — 오류도 로그도 없이
> 「배포했는데 QR 이 여전히 브라우저로 열린다」로만 보인다(2026-09-01 에 넣은 단계다).
...
- [x] **실기기 푸시 종단 검증(운영 서버 대상)** — 지금까지 앱 푸시는 개발 서버로만 검증됐다.
      운영은 FCM 설정만 넣었을 뿐 실제 토큰이 등록된 적이 없다
      🔴 **2026-09-07 첫 시도에서 막혔다.** iOS 발송이 전부 FCM 401 `THIRD_PARTY_AUTH_ERROR` / APNs `BadEnvironmentKeyInToken` — Firebase 에 올린 APNs 키가 **sandbox 전용**이라 스토어 빌드(production APNs)에는 못 보낸다. 상세는 `src/docs/epics/field-feedback-2026-09/README.md` F-14
      ✅ **같은 날 22:25 KST 종료** — Apple Developer 에서 «Sandbox & Production» 키 `5YP6MW3RP8` 을 만들어 Firebase Cloud Messaging 에 교체(FCM 200), 이어서 지령 #51 이 웹·iOS 실기기에 도착하고 iOS 소리까지 확인. 소리는 기기 권한이 소리 없이 잡혀 있어 앱 재설치가 필요했다(트래커 F-15)
