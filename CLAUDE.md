...
**Push (updated 2026-08-09, epic N1 — ships).** **Web push (VAPID) works end to end.** **App push (FCM) is proven end to end on Android** — token registration → FCM 200 → system notification → tap → deep-link landing, all measured on an emulator. **iOS is fully wired but unverified**: APNs key, `GoogleService-Info.plist`, `App.entitlements`, and Background Modes are in place, but the simulator does not issue APNs tokens, so **real iOS delivery needs a physical device**. Key pieces:
...
- **In the app there is no service worker** (WKWebView/Capacitor), so `push.js` branches to `push-native.js` (Capacitor `FirebaseMessaging`). That file owns three things the web gets for free: token registration (done by the **logged-in web page**, never the shell — the shell has no session cookie), tap routing (`notificationActionPerformed`), and a **foreground in-app banner** (`notificationReceived`) — in the foreground the OS delegates display to the app, so without that listener the push is silently dropped.
