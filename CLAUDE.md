...
- ⚠️ Still unverified: **`deploy.sh` has never been run** (no deploy/rollback rehearsal) — OPS-08 is not closed. The rehearsal above only booted the stack; it did not exercise backup → checkout → migrate → restart → rollback.
