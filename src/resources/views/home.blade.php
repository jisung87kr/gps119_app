...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="">
        <div class="h-screen" x-data="mydata" style="padding-bottom: 200px">
            <div class="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 fixed left-0 top-0 right-0 bottom-0 z-[9999] flex items-center justify-center"
                 id="intro"
                 x-show="showIntro"
        ...
        <script>
            const mydata = {
                ...
                showIntro: true,
                loading: false,
                //user: JSON.parse('<?php //echo isset($_SESSION['user']) ? json_encode($_SESSION['user']) : 'null'; ?>//'),
                init(){
                    // if(this.user == '' || !this.user){
                    //     alert('네이버 아이디 인증 후 사용 가능합니다.');
                    //     window.location.href = '/callback/auth/naver.php';
                    // }
                    this.initMap();
                    this.getLocation();
                    setTimeout(() => {
                ...
                shareLocation(type){
                    ...
                    let msg = '';
                    let url = `https://gps119.co.kr/request.php?lat=${this.lat}&long=${this.long}`;
                    switch (type){
                        case 'accident':
                    ...
                    // msg += `\n - 이름: ${this.user.name}\n - 연락처: ${this.user.mobile}`;


                    var params = new URLSearchParams();
                    params.append('message', msg);

                    axios.post('./?api=help', params).then(res => {
                        console.log(res);
                    });
                    alert(msg);
                    this.copyText(url);
