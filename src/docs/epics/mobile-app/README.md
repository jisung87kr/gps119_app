...
- 상태: **N1(서버 푸시 계층) 대부분 완료** — 2026-08-05 착수. N0 블로커 4건은 여전히 «합의·법무»
  - ✅ N0 코드분(0-1~0-2c, 0-8) · ✅ N1 이벤트 배선 + 푸시 계층 + 웹푸시(VAPID) + 관제 딥링크
  - ✅ **웹 푸시는 실브라우저로 종단 검증됨** — 접수 → 알림 표시 → 탭 → 딥링크 착지([`03 §8`](03-push-notifications.md))
  - 🔴 앱 푸시(FCM)는 **명의(M-8)만 대기** — 코드는 있고 자격증명이 없다.
    **2026-08-06: 우회로가 없음이 확인됐다** — 앱 웹뷰에 서비스워커가 없어 웹 푸시로 대체 불가([`03 §6-1`](03-push-notifications.md))
  - 🟡 **N2(앱 셸) 착수** — 셸 생성 + **핵심 가설 검증됨**(웹뷰에서 세션 쿠키·`/broadcasting/auth` 동작).
    앱을 얹은 것만으로 결함 6건이 드러났고 4건은 수정([`07` N2](07-roadmap.md))
  - ✅ **2026-08-06: 「iOS 웹뷰가 위치를 못 쓴다」는 오진이었다.** 원인은 웹뷰가 아니라
    **보안 컨텍스트(http)** 였다 — 로컬 Docker 에 https 를 붙여 해소. **웹 코드는 0줄 바뀌었다**
  - 🔴 실기기 판정 · 앱에서 신고 전송 종단 확인 · N3 이후는 남음
...
> ✅ **2026-08-05 — 이 관문을 통과했다.** N2 에서 셸을 만들어 확인한 결과, 원격 URL 방식이라
> 오리진이 웹과 같아져 **세션 쿠키와 `/broadcasting/auth` 가 그대로 살았다.** 위 §「관문」은
> 「로컬 번들 방식을 택했다면」의 이야기이고, A안을 택한 지금은 발생하지 않는다.
> `createToken` 이 0건인 것도 그대로다 — 필요가 없어서다.
...
| # | 무엇이 문제였나 | 어디에 반영 |
|---|---|---|
| ① | **딥링크 규약이 정작 상황실에게 동작하지 않는다** — `?project=` 를 `/admin/control` 만 읽는다. 행사 controller 는 시스템 롤이 `user` 라 `/control` 로 가고, 행사를 2개 이상 맡으면 엉뚱한 행사가 열린다 → ✅ **N1 에서 해소** | [03 §5-1](03-push-notifications.md) · [07](07-roadmap.md) N1 + 게이트 |
