...
- [ ] **2026-09-08 — App Store 재심사 제출**: 빌드 2 가 **2.1 Information Needed** 로 돌아왔다(기능 지적 없음, 정형 6문항). 회신(실기기 녹화·데모 계정 3종·영문 답변)과 함께 **빌드 5** 로 교체 제출 → `docs/store/app-review-reply-2026-09-08.md`. 🔴 조사 중 발견: 지령 이력 있는 구급대원·상황실 계정은 **계정 삭제가 500**(FK RESTRICT) → ✅ 같은 날 익명화로 수정(ADR-0010). 데모 계정 3종은 `review:demo` 명령. **남은 것: 배포 → 운영에서 `review:demo` → 녹화 → 회신**
