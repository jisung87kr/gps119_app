...
| F-15 | (현장 발견) 소리·배너가 안 남 — 설정 → 알림 목록에 GPS119 만 「배지」 | **뱃지 플러그인 `clear()` 가 iOS 에서 `.badge` 만으로 권한을 요청**하고, 웹이 앱을 열 때마다 그걸 불러 새 설치의 첫 프롬프트가 배지뿐이 됐다. 그 뒤 alert·sound 요청은 iOS 가 무시 | 웹 | ✅ 09-07 PR #43 배포 `d5c8694` · 기기는 삭제→재시동→재설치 필요 |
...
- [ ] F-15 현장 재확인: 아이폰 앱 삭제 → 재시동 → 재설치 → 알림 받기 → 설정 → 알림 목록에 GPS119 「배너, 사운드, 배지」 → 배정 → OS 푸시 배너+소리
