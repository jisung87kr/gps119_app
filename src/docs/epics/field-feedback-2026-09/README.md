...
| F-03 | 아이폰 새로고침 방법 | 앱엔 양 OS 모두 없음. iOS 당겨서 새로고침 추가 | 셸 | 빌드 3 ❌(Capacitor 가 바운스를 꺼 둠) → **빌드 4** 수정 `bf461b4` · 실기기 확인 대기 |
...
- [ ] 빌드 4 설치 → 목록 화면 당겨서 새로고침 · 지도 화면을 끌 때 오작동 없음 · `app_version` `1.0 (4)`
...
| 항목 | 기기 | 확인할 것 |
|---|---|---|
| F-03 | iPhone **빌드 4** | 목록 화면에서 당겨서 새로고침이 되는가 · **지도 화면을 끌 때 오작동하지 않는가** (빌드 3 은 바운스 꺼짐으로 불가) |
