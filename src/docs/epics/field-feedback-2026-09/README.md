...
| F-11 | iOS: 알림 켜도 계속 꺼짐으로 표기 | **09-04 배포에서 고친 증상과 동일** — 재확인. 단 운영 iOS 푸시 종단 미검증 | 재확인 | ☐ |
...
**09-04 배포(`eb2cf8b`)에서 고친 증상과 같다.** 켜짐 기록이 모듈 변수뿐이라 화면을 옮기면 「알림 꺼짐」으로 읽혔다(원격 URL 셸은 이동마다 새 번들). 지금은 `localStorage` 에 남기고 다른 페이지에서 끌 때 토큰을 다시 받아 서버에 알린다.

- ☐ 고객 재확인(09-04 이후 빌드로)
- 🔴 재확인 때 **실제 수신까지** 본다. `DEPLOY.md` §5 에 「실기기 푸시 종단 검증(운영 서버 대상)」이 미완으로 남아 있다 — 앱 푸시는 개발 서버로만 검증됐다.
- 그래도 안 되면 다음 용의자: iOS 에서 APNs 토큰 확보 전 `getToken()` 실패 → 화면엔 「알림 등록에 실패했습니다. 네트워크를…」 문구가 뜬다(꺼짐 표기와는 다르다).
...
2. F-11 — 09-04 이후 빌드로 다시 봐도 알림이 「꺼짐」으로 보이는지, 그리고 실제로 알림이 «오는지»
3. F-06 — 검색이 필요했던 화면이 관제 배정 패널인지 관리자 「참가자 추가」인지 (둘 다 넣었으니 확인만)
4. F-09 — 그 안드로이드 기기에 카카오내비가 있는지, 카카오맵만 있는지
5. F-07 — 합성 사이렌으로 갈지, 원하는 음원이 따로 있는지
...
| F-11 | iPhone, **운영 서버** | 알림 켜기 → 실제 수신 (`DEPLOY.md` §5 「운영 대상 iOS 푸시 종단 검증」 미완) |
