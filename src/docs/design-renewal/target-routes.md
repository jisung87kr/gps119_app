...
두 안을 만들어 같은 데이터로 비교한 뒤 **안 A 를 채택**했다.

| | 안 A — Ink + Brand **(채택)** | 안 B — Field Console (미채택) |
|---|---|---|
| 명도·주색 | 라이트 · 페트롤 틸 `#0E6E7C` | 다크 · 하이비스 앰버 `#FFC400` |
| 하단 공간 | 이동 탭 바 3개 | 상시 긴급 버튼 독 |
| 이동 내비 | 하단 탭 바 | 상단 아이콘 바 |
| 면 | 라운드 2xl 카드 | 엣지투엣지 플랫 행 + 좌측 상태 색 바 |
| 통계 | 2x2 카드 | 4칸 가로 계기판 스트립 |

안 B 를 접은 이유로 확인된 것:
- **카카오맵은 라이트 타일**이라 다크 셸과 명도 차가 커서 지도 화면에서 지도가 튄다.
- **직사광선 가독성**은 라이트가 유리하다. 마라톤·야외 축제 낮 시간대가 주 사용 시간이다.
- **상시 빨간 독**은 오조작 위험과 심리적 부담이 남는다.

안 B 의 아이디어 중 다시 꺼내볼 만한 것 (지금은 미적용):
- 목록 행 **좌측 상태 색 바** — 배지 글자를 읽지 않고 스크롤 중에 상태를 훑을 수 있다.
- 통계 **고정폭 숫자(`tabular-nums`)** — 값이 갱신될 때 숫자 폭이 흔들리지 않는다.

안 B 브랜치(`feature/user-ui-renewal-b`)와 상세 비교 문서
(`variant-b-field-console.md`)는 그 브랜치에만 있다. 참고가 끝나면 삭제하면 된다.
